...
/* Defines */
#define CLASS_NAME L"thToolbar"
#define WIN32_CLASS_NAME L"ToolbarWindow32"
...
void thToolbarText::operator=(thString in) {
    ...
    m_pParent->m_text = in;
    m_pParent->m_changedata.pszText = (LPWSTR)m_pParent->m_text.c_str();
...
thToolbarItem::thToolbarItem(thToolbar * a_parent) : m_pParent(a_parent), OnClick(NULL) {
    ...
    m_data.iString = (INT_PTR)L"TBButton";
...
LRESULT thToolbarItem::processCommandMessage(UINT a_uMsg, WPARAM a_wParam, LPARAM a_lParam)
{
    ...
    MSG_LOG(L"Toolbar Item CLICKED, Id=%d", this->m_id);
...
void thToolbarItemList::Add(thString a_text) {
    ...
    if (0 != m_pParent) {
        ...
        if (newToolbar) {
            MSG_SUCCESS(L"Toolbar item created with id=%d", newToolbar->m_id);
